...
- conftest.pyがテスト環境の初期設定と終端処理を司る。
...
- htmlcovのindex.htmlがPytestの結果。このファイルのカバレッジ100%を目指してテストファイルを作っていく。

### DDD (Domain Driven Design)
- 「アプリケーションに必要と思しき値は他の値の演算から得られないか？」を主眼に、根幹の値(intやstr)を抽出する。
- この値はクエリの抽出結果やClass()の初期化でしか発生させないようにする。以降、値の演算と操作だけでアプリケーションを作っていく。
- 値の演算と操作とは、getterの種類を多様にしていく。ある値を渡し、それ以上かどうかを判断する.is_above(arg)のような関数。
- 「値と多様なgetter」を値オブジェクトと呼び、DDDの根幹となる。
- 複数の値オブジェクトで構成されるものがエンティティ。エンティティはクエリの実行結果を受けるなど、値エンティティの容器として機能する。
- 値オブジェクトとエンティティを修飾するクラスがドメインサービス。「これはログインユーザーのID」など、特有の意味を持たせる。
- リポジトリはデータベースや外部APIに接続する。
- ユースケースは値オブジェクト・エンティテイ・ドメインサービス・リポジトリを組み合わせてアプリケーションの処理の実態を作る。
- ビューはルーティングとDDDの境界でリクエストボディからエンティティを作ったり、逆にエンティティからレスポンスボディを作る。
- スキーマはリクエスト・レスポンスを規定。intやstrなどプリミティブ型で構成する。
