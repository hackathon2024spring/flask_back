...
class UserRepositoryImpl(UserRepository):
    ...
    async def upsert_exercises_selected(
        ...
        async for session in self.session_scope():
            for exe in exercises_selected:
                stmt = (
                    sqlite_insert(OrmExerciseSelected)
                    .values(
                        user_id=exe.user_id(),
                        exercise_id=int(exe.id()),
                    ...
                    .on_conflict_do_update(
                        ...
                        set_=dict(selected=exe.selected()),
                    )
                )
    ...
    async def get_exercises_done(
        ...
        async for session in self.session_scope():
            ...
            query = (
                ...
                .where(
                    (done_subquery.c.exercise_id != None)
                    | (selected_subquery.c.exercise_id != None)
                )
                .order_by(OrmExercise.id)
            )
    ...
    async def get_exercises_this_month(self, uid: str, calendar: CalendarRequest):
        ...
        async for session in self.session_scope():
            ...
            # 既存の一時テーブルを削除
            await session.execute(text("DROP TABLE IF EXISTS date_range;"))

            # 一時テーブルの作成
            await session.execute(text("CREATE TEMPORARY TABLE date_range (day DATE);"))
            ...
            query = (
                select(
                    ...
                    case(
                        (func.max(OrmExerciseDone.done) == 1, literal(True)),
                        else_=literal(False),
                    ).label("exerciseDone"),
                )
